...
delete from VENDA where VENDA_nID > 1;

SELECT VENDA_nID FROM VENDA WHERE VENDA_cSTATUS LIKE 'PENDENTE' AND CLI_nCOD = 1;


 INSERT VENDA (VENDA_nVLRTOTALVENDA, VENDA_dtDTCOMPRA, VENDA_cCODRASTREIO, VENDA_cSTATUS, CLI_nCOD) VALUES ( 0, NOW(), 'xxx', 'PENDENTE', 1);
 
 select C.CATEGORIA_nID, C.CATEGORIA_cDESC from PRODUTO_CATEGORIA PC INNER JOIN CATEGORIA C ON C.CATEGORIA_nID = PC.CATEGORIA_nID WHERE PC.PRODUTO_nID = 1 and CATEGORIA_cSTATUS = 'ATIVO';
